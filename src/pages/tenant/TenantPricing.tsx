...
const T = {
    es: {
        ...
        subtitle: 'Herramientas de perfilamiento conductual para acompañar a tus deportistas.',
        monthly: 'Mensual',
        annual: 'Anual',
    ...
    en: {
        title: 'Plans & pricing',
        subtitle: 'Behavioral profiling tools to support your athletes.',
        monthly: 'Monthly',
        annual: 'Annual',
        save: 'Save up to 21%',
        year: 'year',
    ...
    pt: {
        ...
        subtitle: 'Ferramentas de perfilamento comportamental para acompanhar seus atletas.',
        monthly: 'Mensal',
        annual: 'Anual',
...
export const TenantPricing: React.FC = () => {
    ...
    return (
        ...
        >
            ...
            {/* Trial callout */}
            {currentPlan === 'trial' && (
                <div className="bg-argo-violet-50 border border-argo-violet-100 rounded-xl px-5 py-3.5 text-center mb-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-10">
                ...
                {/* PRO */}
                <div className={`bg-white rounded-[16px] shadow-argo p-7 flex flex-col ${currentPlan === 'pro' ? 'ring-2 ring-argo-violet-500' : ''}`}>
                    <p className="text-[11px] font-bold text-argo-violet-500 uppercase tracking-[0.08em] mb-2">PRO</p>
                    <div className="mb-1">
                        <span className="text-[34px] font-bold text-argo-navy tracking-tight">
                            {annual ? '$40' : '$49'}
                        </span>
                    ...
                    {annual ? (
                        <>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[11px] font-bold text-argo-violet-500 bg-argo-violet-50 px-2 py-0.5 rounded-md">{lang === 'en' ? 'Save 18%' : lang === 'pt' ? 'Economize 18%' : 'Ahorra 18%'}</span>
                            </div>
                            <p className="text-xs text-argo-light mb-4">{t.billedAnnually} ($480/{t.year})</p>
                        </>
                    ) : (
                        <p className="text-xs text-argo-light mt-1.5 mb-4">{lang === 'en' ? 'Billed monthly' : lang === 'pt' ? 'Cobrança mensal' : 'Facturación mensual'}</p>
                    )}
                ...
                {/* ACADEMY */}
                <div className={`bg-white rounded-[16px] shadow-argo p-7 flex flex-col relative ${currentPlan === 'academy' ? 'ring-2 ring-argo-violet-500' : 'border-2 border-argo-violet-500'}`}>
                    ...
                    {annual ? (
                        <>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[11px] font-bold text-argo-violet-500 bg-argo-violet-50 px-2 py-0.5 rounded-md">{lang === 'en' ? 'Save 21%' : lang === 'pt' ? 'Economize 21%' : 'Ahorra 21%'}</span>
                            </div>
                            <p className="text-xs text-argo-light mb-4">{t.billedAnnually} ($840/{t.year})</p>
                        </>
                    ) : (
                        <p className="text-xs text-argo-light mt-1.5 mb-4">{lang === 'en' ? 'Billed monthly' : lang === 'pt' ? 'Cobrança mensal' : 'Facturación mensual'}</p>
                    )}
            ...
            {/* Families — compact */}
            <div className="pt-8 border-t border-argo-border">
