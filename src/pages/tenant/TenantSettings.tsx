            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-[14px] p-6 shadow-argo space-y-6">
                    ...
                    <div className="space-y-5">
                        ...
                        <Field label={o.nombreInstitucion}>
                            <input className={inputClass} value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder={o.nombrePlaceholder} />
                        </Field>

                        <Field label={o.logo}>
                            <div className="flex items-center gap-4">
                                {logoPreview ? (
                                    <img src={logoPreview} alt="Logo" className="w-14 h-14 rounded-[10px] object-contain border border-argo-border bg-argo-neutral" />
                                ) : (
                                ...
                                <div>
                                    <button type="button" onClick={() => fileRef.current?.click()} className="text-[13px] font-medium text-argo-violet-500 hover:text-argo-violet-600 transition-colors">
                                        {logoPreview ? o.cambiarLogo : o.subirLogo}
                                    </button>
                                    <p className="text-[11px] text-argo-light mt-0.5">JPEG, PNG o WebP</p>
                        ...
                        <Field label={o.tipoInstitucion}>
                            <div className="flex flex-wrap gap-2">
                                {TIPOS.map(v => <ChipButton key={v} selected={tipo === v} onClick={() => setTipo(tipo === v ? '' : v)}>{o.tipoLabels[v]}</ChipButton>)}
                            </div>
                        </Field>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Field label={o.pais}>
                                <select className={selectClass} value={country} onChange={e => setCountry(e.target.value)}>
                                    ...
                                    {PAISES.map(v => <option key={v} value={v}>{o.paisLabels[v]}</option>)}
                                </select>
                            </Field>
                            <Field label={o.ciudad}>
                                <input className={inputClass} value={city} onChange={e => setCity(e.target.value)} placeholder={o.ciudadPlaceholder} />
                            </Field>
                        </div>
                    </div>
                    ...
                    <div className="space-y-5">
                        ...
                        <Field label={o.nombreCompleto}>
                            <input className={inputClass} value={fullName} onChange={e => setFullName(e.target.value)} placeholder={o.nombreCompletoPlaceholder} />
                        </Field>
                        ...
                        <Field label={o.rolEnInstitucion}>
                            <select className={selectClass} value={role} onChange={e => setRole(e.target.value)}>
                                ...
                                {ROLES.map(v => <option key={v} value={v}>{o.rolLabels[v]}</option>)}
                            </select>
                        </Field>
                    </div>
                    ...
                    <div className="flex justify-end pt-2">
                        <button
                            ...
                            onClick={handleSave}
                            disabled={saving}
                            className="px-6 py-2.5 rounded-xl text-[13px] font-semibold bg-argo-violet-500 text-white hover:bg-argo-violet-600 transition-colors disabled:opacity-60"
                        >
                ...
                <div className="bg-white rounded-[14px] p-6 shadow-argo">
                    ...
                    <div className="pt-4 border-t border-argo-border">
                        <p className="text-[11px] font-semibold text-argo-light uppercase tracking-[0.08em] mb-2.5">{dt.settings.idioma}</p>
