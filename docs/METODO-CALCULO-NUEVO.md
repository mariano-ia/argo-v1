...
> Documento autoritativo del método reformado, **ronda 4 (pulido final)**. Parte de la v3 (ronda 3: balde A + balde B + decisión motor M1 + decisión veta opuesta O1 + cambios menores X1-X4, sobre la v2 del panel) y aplica encima el **pulido final R4-* (PILA 1 de arreglos técnicos + copy, y PILA 3 de consolidación de límites y agenda empírica)**. Estructura numerada idéntica (secciones 0-15) para comparar manzana con manzana; se preserva todo lo que la ronda 4 no toca. Es la referencia de implementación y de re-revisión (reemplaza la v3, conservada en git).
>
