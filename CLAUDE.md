...
**Registered mark ® + joined wordmark (STRICT, aligned 2026-07-02):** every consumer brand name is written **joined (NO spaces)** and carries a **trailing ®** on every place it is *displayed*: **`ArgoOne®`**, **`ArgoOne+®`**, **`ArgoPuente®`**, **`ArgoMethod®`** (and `ArgoAcademy®`). This now **includes the company mark**: the old spaced two-weight "Argo Method" logo becomes **`ArgoMethod®`** (still Argo `font-weight: 800` + rest thin `300` web / `200` deck; the **"+" stays 800**; the **® is thin and comes last** — after the "+" in `ArgoOne+®`). Applies to the **frontend (`src/`)**: wordmark spans, plain prose, labels, titles, legal pages (Terms/Privacy), report and checkout copy. **Do NOT add ® to** (and keep the spaced "Argo Method" there): (a) code identifiers / component names (`ArgoOneLanding`, `AdminArgoOne`, `PuentesFlow`), i18n keys, routes, DB object names; (b) JSON-LD / schema.org `name` fields (the legal-entity string stays `"Argo Method"`); (c) email sender/From display-names (`from: 'Argo Method <hola@…>'`). **Backend `api/` (email templates, crons) is NOT yet aligned** — it still carries the old spaced forms ("Argo Puentes", "Argo Method", "ArgoOne"); align it in a dedicated follow-up before it matters for sends. In emails the product header is the `ArgoOne®` wordmark, never "Argo Method One"; `ArgoMethod®` is the **company** mark used ONLY for company / tenant / Puente / report emails, never for the ArgoOne product emails (purchase confirmation, play invite, panel access). As-built + the deterministic script: `docs/BRAND-NAMING.md`.
